...
| 切片 | 表数 | mean TEDS_X |
|---|---|---|
| OmniDocBench 表格(前 25,人工真值 span 口径) | 25 | **0.812** |
...
| **OmniDocBench(人工真值)** | **span 口径** | **0.812** |

**这就是"换 benchmark"的全部意义**:模型没变,把尺子从"和压扁口径的系统输出比一致度"换成"和 span 口径的人工真值比准确度",同一个 UniRec 从 0.526 跳到 **0.812**——0.526 是口径假象,0.812 才是真实表识别能力。

样例(`page-f9583127`,5×N 含数学的表):我方 HTML 与 GT **逐格一致**(`Dimension (D)`/`768 64 64 32 128`…),TEDS 0.995;差异仅数学定界符与个别 OCR 字(`d_ν` vs `d_v`)。分布大半 0.85–1.0,少数失败例(复杂表/AR 退化,如 0.007/0.444)拉低均值——与论文"0.1B 模型在难表上仍有失败"一致。
