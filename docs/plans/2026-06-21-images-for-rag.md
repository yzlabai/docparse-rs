...
> **状态：已实施（2026-06-21）**，见 [devlog](../devlogs/2026-06-21-images-for-rag.md)。§3 范围全部完成 + **PPTX（commit 4）+ HTML（commit 5）抽图、版面/tagged Caption 绑定（commit 6）已补**。图片→RAG 现覆盖 PDF / DOCX / PPTX / HTML；caption 四档来源（caption-line / layout-caption / alt / vlm）。仅剩 "表格单元格内图片" 列入后续。
>
