...
>
> **状态(2026-06-17):§1+§2+§3 全部实现并验证**:
> - **§1 消除静态化**:`prepare.py` 整条 Python 链已删,raw HF ONNX 经 `onnx_loader`(`with_ignore_value_info`)直载;v4 零回归、v6 输出与旧静态化版逐字一致(含 `、`)。
> - **§2 字典**:`load_dict`/`parse_yml_char_dict` 从 rec yml 抽(零 yaml 依赖,带单测)。
> - **§3 自动下载**(用户定策=**首次交互确认**):`crates/docparse-ocr/src/fetch.rs`(ureq,带 3 次重试抗 HF CDN 中断 + User-Agent + 原子 rename);CLI `ensure_ocr_models` 在缺模型时 TTY 下 y/N 确认、非 TTY(MCP/REST/管道/CI)报清晰错、`DOCPARSE_OCR_DOWNLOAD=1` 预确认。**实测**:空 `ppocr-v6` 目录下 `DOCPARSE_OCR_DOWNLOAD=1 --ocr` 自动拉 4 文件(~7MB)后端到端 OCR 正确。
>
> 全测试+clippy 过(fmt:仅触碰本次新增,既存 drift 未扫)。
...
| 决策 | 选项 | 倾向 |
|---|---|---|
| **自动下载是否默认开** | (a) 默认自动拉;(b) 缺模型时报清晰错+提示命令;(c) 首次交互确认 | **已定=(c) 首次交互确认**(用户选):TTY 下 y/N 提示,确认才下;非 TTY(server/管道/CI)退化为清晰错;`DOCPARSE_OCR_DOWNLOAD=1` 给自动化预确认。TTY 检测天然区分三接口,无须 per-face 配置 |
