...
### I1 · 版面模型整批/整服务只载一次（M1，优先级 P0）

**问题**:`--layout` 及 `--formula-model`/`--transcribe-model` 内部用到的版面检测,**仍每文件从路径加载 `LayoutModel`**——批量多 PDF 反复吃 ONNX 加载;**连服务端(MCP/REST)也每请求重载**(`EnhanceState::apply` 传的是 `&self.layout_model` 路径)。Phase 9 收口时只覆盖了 OCR + UniRec,版面留下了。

**方案**:把 docparse-ocr 三个公有函数从"收路径、内部 `LayoutModel::new`"改为"**收预载 `&LayoutModel`**":
...
调用侧:
- CLI:`RunModels` 加 `LazyLayout`(`OnceLock<Result<LayoutModel,String>>`,键即 `cli.layout_model`),`parse_and_enhance` 三处从缓存取。**惰性**——不带这些 flag 的批量永不加载。
- 服务端:`EnhanceState` 加惰性 `LayoutModel`(同 `unirec` 的 `OnceLock<Arc<…>>` 模式),顺带让**服务也只载一次**(现状是白白每请求重载,本项免费修掉)。

**影响面**:3 个签名 + 6 处调用(CLI ×3、EnhanceState ×3)。`LayoutModel` 已 `pub`。
**风险**:中——动到服务端路径。缓解:现有 mcp/server 单测 + `--layout` 单文件/批量手测,产出与改前逐字节对照。
**收益**:批量 `--layout`/公式/转写不再反复载版面模型;服务端顺带提速。**这是 Phase 9 模型复用真正的收尾。**

### I2 · 递归遇符号链接环不爆栈（M1，P1，review 暴露）

**问题**:`collect_dir` 用 `path.is_dir()` 判断是否递归,**`is_dir()` 跟随符号链接**;`-r` 扫到符号链接环(`a/link → a`)会**无限递归 → 爆栈**。违反项目"不 panic"红线(虽属病态输入)。

**方案**:递归前跳过符号链接目录——`if path.is_dir() && !path.is_symlink()`(`Path::is_symlink` 自 1.58 稳定)。即**默认不跟随符号链接目录**(更安全的默认);如需跟随,后续可加 `--follow-symlinks` + canonicalize 访问集去环。
**影响面**:[batch.rs](../../crates/docparse-cli/src/batch.rs) `collect_dir` 一行 + 一条测试(造环验证不挂)。
**风险**:低。**effort:S。**
