...
- [ ] **G9a · Tagged PDF 结构读取(性价比最高,先做)**:带无障碍标签的 PDF 自带作者声明的结构树——等于免费真值。
  - 解释器跟踪 marked-content(`BDC /P <</MCID n>> … EMC`),TextChunk 记 MCID;
  - 解析 `StructTreeRoot`(StructElem 的 `/S`:H1–H6/P/L/LI/Table/TR/TD…),MCID → 结构类型映射;
  - 有标签文档:标题层级/列表/表格直接取标签(几何法降为兜底);`/MarkInfo /Marked true` 探测入画像。
  - 验收:找/构造 tagged 样例,标签结构正确入 IR;无标签文档零变化。
