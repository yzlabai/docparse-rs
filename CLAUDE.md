...
战略见 [docs/roadmap.md](docs/roadmap.md)；近期里程碑 N1–N6 见 [docs/plans/next-iteration.md](docs/plans/next-iteration.md)（M1–M7 历史见 [docs/plans/beating-docling.md](docs/plans/beating-docling.md)）。**已完成**（2026-06-11，见 docs/devlogs/）：M1–M7 + N1–N5 + Phase 4 可自主项全部——G1（格式 3→11：XLSX/PPTX/MD/CSV/SRT·VTT/LaTeX/EML/PNG·JPEG）、G2/G4/G8a、G8b（VLM 图片描述 + `--vlm-tables`，mock 验收）、G6（Python 客户端 + LangChain/LlamaIndex loader，五行验收实测）、G7（1847 输入压测 + fuzz 四目标 ~1020 万次执行均零 panic/崩溃）、G9 全部（含 G9d 表结构：通道列+规则线分带+二分重试，TEDS 验收门过）。**记分牌**（born-digital LTR）：vs ODL NID 0.792/MHS 0.685/TEDS 0.419；vs Docling NID 0.822/MHS 0.643/TEDS 0.474。**剩余池**（候外部输入）：PyPI/crates.io 发布（账号）、Ollama 真实服务验收、arXiv 千份压测与 fuzz 24h（资源/排期）、AsciiDoc/JATS/RTL/MCP-REST 新能力透传（按需）；详见 [docs/plans/closing-docling-gaps.md](docs/plans/closing-docling-gaps.md)。⚠️ 经验：记分牌大跳几乎全是评测/输出管线 bug——分数可疑先怀疑管线（G9d 行对齐 DP：尺子不修,结构改进不可见）；管道退出码会掩盖失败（fuzz 构建失败被 tail 吞过）；新格式 e2e 是共享层的免费测试矩阵（synth 段距/列表语义两个横向 bug 都由此暴露）。
