...
1. 先**写计划**——含需求三件套、范围与"不做什么"、**用户使用例子**、**测试用例**、验收标准。记进当轮 `docs/devlogs/YYYY-MM-DD-<topic>.md` 开头（**不再单设 plans/**，计划与过程同篇 devlog）。
2. **review 相关文档**（devlog 计划段 + 关联 status/capabilities），确认方向、落点、不变量无误，再动手。
3. **实施**（源码 + 测试代码同 PR；偏离计划就回写 devlog）。
4. **测试**——跑单测 + §1 跨样例回归；有问题就**修复并 review**，循环直到完善、无遗留问题。
5. **更新相关文档**（功能说明/capabilities + devlog 记完成情况 + 必要的 status）。
...
`docs/` 权威总览：`architecture.md`（系统架构）、`capabilities.md`（功能/能力清单）、`status.md`（进度/记分牌单一真源）、`roadmap.md`（战略/愿景）、`agent-integration.md` + `agent-enhancement-decisions.md`（接入与增强决策）、`iteration-guide.md`（怎么开发）。`refer/`/`analysis/`（调研与关键决策论证，如 tract 补丁）。**历史回溯只用 `devlogs/`**（每项工作一篇过程记录）——已不再单设 `plans/`、`testresults/` 目录（旧内容已删，回溯靠 devlog）。新工作过程记录补 `docs/devlogs/`，命名与模板见 AI_AGENT_DEV_SPEC §4–5。
...
- **历史回溯（计划 + 过程 + 结果）**：[docs/devlogs/](docs/devlogs/)（唯一回溯记录）
