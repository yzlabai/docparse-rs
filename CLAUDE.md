...
战略见 [docs/roadmap.md](docs/roadmap.md)；近期里程碑 N1–N6 见 [docs/plans/next-iteration.md](docs/plans/next-iteration.md)（M1–M7 历史见 [docs/plans/beating-docling.md](docs/plans/beating-docling.md)）。**已完成**（2026-06-10，见 docs/devlogs/）：M1–M7 全部 + N1 评测（vs ODL NID 0.764/MHS 0.627、vs Docling NID 0.833/MHS 0.645，clean LTR 达同台水平）+ N2 服务化（MCP stdio + REST，四接口逐字节一致）+ N4 大部（表格四检测器）+ N5 安全预检与画像（隐藏文本过滤 + zip-bomb/页数守卫 + 页级复杂度画像）+ N3 真实 enhancer（P4 路线：ONNX 内嵌 PP-OCRv4 × tract 纯 Rust，`--ocr`，chinese_scan 14/14 行、数字页零模型）。**近期里程碑全部收官、十大模块全部闭合**（综合测评见 docs/testresults/2026-06-10-benchmark-roundup.md）；剩项属确定性天花板（CJK 版面/无框表 recall，P1c 已证放宽门必回归）或远期（N5c 画像、人工真值集）。⚠️ 经验：记分牌大跳几乎全是评测/输出管线 bug——分数可疑先怀疑管线（详见 devlogs/2026-06-10-session-summary）。
