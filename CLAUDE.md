...
战略见 [docs/roadmap.md](docs/roadmap.md)；近期里程碑 N1–N6 见 [docs/plans/next-iteration.md](docs/plans/next-iteration.md)（M1–M7 历史见 [docs/plans/beating-docling.md](docs/plans/beating-docling.md)）。**已完成**（2026-06-10，见 docs/devlogs/）：M1–M7 全部 + N1 评测（vs ODL NID 0.780/MHS 0.597、vs Docling NID 0.818/MHS 0.634(Form 流解释后)，clean LTR 达同台水平）+ N2 服务化（MCP stdio + REST，四接口逐字节一致）+ N4 大部（表格四检测器）+ N5 安全预检与画像（隐藏文本过滤 + zip-bomb/页数守卫 + 页级复杂度画像）+ N3 真实 enhancer（P4 路线：ONNX 内嵌 PP-OCRv4 × tract 纯 Rust，`--ocr`，chinese_scan 14/14 行、数字页零模型）。**近期里程碑全部收官、十大模块全部闭合**（综合测评见 docs/testresults/2026-06-10-benchmark-roundup.md）。**下一阶段** Phase 4（G1–G7，补齐 Docling 占优轴：版面/表结构 ONNX enhancer、XLSX/PPTX、OCR 长尾、生态接入、语料压测）见 [docs/plans/closing-docling-gaps.md](docs/plans/closing-docling-gaps.md)。⚠️ 经验：记分牌大跳几乎全是评测/输出管线 bug——分数可疑先怀疑管线（详见 devlogs/2026-06-10-session-summary）。
